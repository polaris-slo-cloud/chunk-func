...
import { AccumulatedStepInput, WorkflowStep } from './step';
import { WorkflowGraph } from './workflow-graph';

/**
 * A strategy for choosing a resource profile configuration for a particular workflow step, given the current state and input.
 */
export interface ResourceConfigurationStrategy {
    ...
    /**
     * Chooses the resource profile configuration for the specified step, given the current state and input.
     */
    chooseConfiguration(workflowState: WorkflowState, step: WorkflowStep, input: AccumulatedStepInput): ResourceProfile;

}

/** Factory for creating ResourceConfigurationStrategies */
export type ChooseConfigurationStrategyFactory = (graph: WorkflowGraph) => ResourceConfigurationStrategy;
